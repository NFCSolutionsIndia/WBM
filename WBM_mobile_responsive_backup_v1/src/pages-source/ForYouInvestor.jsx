...
const ForYouInvestor = () => {
  ...
  const streams = [
    ...
    { id: "02", title: "Crushed Powders", desc: "Aluminium powder → aluminium mfrs. Plastic powder → plastic mfrs.", icon: <Zap size={24} /> },
  ...
  const dashboardStats = [
    { label: "Carbon Offset", value: "127.4", unit: "MT CO2e", trend: "+12% QoQ" },
    { label: "Gold Recovered", value: "18.6", unit: "kg", trend: "+4% QoQ" },
    { label: "Lithium Recovered", value: "2,340", unit: "kg", trend: "+9% QoQ" },
    { label: "Rare-Earth Recovered", value: "856", unit: "kg", trend: "+15% QoQ" },
  ];
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 relative z-10 text-center">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-white/5 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 text-[var(--c-lime)] mb-8">
              ...
              <span className="text-[10px] md:text-xs font-bold uppercase tracking-[0.2em]">Investor Journey</span>
            </div>
            
            <h1 className="text-4xl md:text-[64px] font-black text-white leading-[0.9] tracking-tighter mb-8 uppercase">
              For <span className="text-[var(--c-lime)] text-glow">Investors.</span>
            </h1>
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Button href="/investor/pitch-deck" size="lg">View Pitch Deck</Button>
              <Button href="/investor/book-call" variant="outline" size="lg">Book Investor Call</Button>
            </div>
          </motion.div>
        </div>
      </section>
                ...
                <div className="space-y-6">
                   {[
                     ...
                     <div key={i} className="flex gap-4 group">
                        <div className="text-[var(--c-lime)] font-black text-xl group-hover:translate-x-2 transition-transform">→</div>
      ...
      <section className="py-24 bg-[var(--c-fg)]/5">
        <div className="max-w-7xl mx-auto px-6">
           <div className="text-center mb-20">
              <h2 className="section-title text-[var(--c-fg)] mb-6">4×5 <span className="text-[var(--c-lime)]">Economics.</span></h2>
      ...
      <section className="py-24 bg-[var(--c-bg)]">
        <div className="max-w-7xl mx-auto px-6">
           ...
           <div className="mt-12 text-center">
              <Button href="/circular-economy" variant="outline">Open the full 4×5 matrix →</Button>
           </div>
        </div>
      </section>

      {/* Proof Points Stats */}
      <section className="py-24 bg-[var(--c-fg)] relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
           <h2 className="text-sm font-black tracking-[0.3em] text-[var(--c-bg)]/50 uppercase mb-16">The numbers that matter</h2>
           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
              {stats.map((stat, i) => (
                <motion.div 
                  ...
                  transition={{ delay: i * 0.1 }}
                >
                   <div className="text-6xl md:text-7xl font-black text-[var(--c-lime)] tracking-tighter mb-4 leading-none">{(stat.value === 0 || stat.value === "0" || !stat.value) ? "--" : stat.value}{(stat.value === 0 || stat.value === "0" || !stat.value) ? "" : stat.suffix}</div>
                   <div className="text-[10px] font-bold text-[var(--c-bg)] uppercase tracking-[0.2em]">{stat.label}</div>
                </motion.div>
              ))}
           </div>
           <p className="mt-16 text-[var(--c-bg)]/40 text-xs font-medium uppercase tracking-widest italic">All financial figures are indicative — definitive numbers in the data room.</p>
        </div>
              ...
              {[
                ...
                >
                   ...
                   <div className="flex items-baseline gap-2 mb-2">
                      <span className="text-4xl font-black text-[var(--c-fg)] group-hover:text-[var(--c-lime)] transition-colors">{(stat.value === 0 || stat.value === "0" || !stat.value) ? "--" : stat.value}</span>
                      <span className="text-xs font-bold text-[var(--c-fg2)]">{(stat.value === 0 || stat.value === "0" || !stat.value) ? "" : stat.unit}</span>
                   </div>
      ...
      <section className="py-24 bg-[var(--c-bg)] text-center border-t border-[var(--c-border)] relative overflow-hidden">
        <div className="max-w-4xl mx-auto px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="section-title text-[var(--c-fg)] mb-10 leading-none">Pitch Deck, Model, <br /> <span className="text-[var(--c-lime)]">Handshake.</span></h2>
            <p className="text-xl text-[var(--c-fg2)] font-medium mb-12 max-w-2xl mx-auto">Access the full data room, IP summaries, and forward-looking pipelines gated by your verified identity.</p>
            <div className="flex flex-wrap justify-center gap-4">
              <Button href="/investor/pitch-deck" size="lg">Request Data Room Access</Button>
            </div>
          </motion.div>
        </div>
      </section>
