...
const Insights = () => {
  ...
  // ... (categories and insights remain the same)
        ...
        <section className="pt-24 pb-0 relative z-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <motion.div 
              ...
              viewport={{ once: true }}
              className="bg-[var(--c-bg)] rounded-[10px] overflow-hidden shadow-2xl flex flex-col lg:flex-row border border-[var(--c-border)]"
            >
              <div className="lg:w-1/2 h-[400px] lg:h-auto overflow-hidden">
                <img src={featuredInsight.image} alt={featuredInsight.title} className="w-full h-full object-cover" />
              </div>
              <div className="lg:w-1/2 p-10 lg:p-16 flex flex-col justify-center">
                ...
                <div className="flex items-center gap-8 text-sm text-gray-400 font-bold mb-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="flex flex-wrap items-center gap-4">
              {categories.map((cat) => (
                <button
                  ...
                  activeCategory === cat 
                  ? 'bg-[var(--c-fg)] text-[var(--c-bg)] shadow-lg' 
                  : 'bg-[var(--c-fg)]/5 text-[var(--c-fg)]/70 hover:bg-[var(--c-fg)]/10'
                }`}
                >
                  {cat}
                </button>
            ...
            <div className="relative w-full md:w-72">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-3 bg-gray-50 dark:bg-white/5 dark:bg-white dark:bg-[#121214]/5 border border-gray-200 dark:border-white/5 dark:border-white/10 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-[var(--c-lime)]/20 transition-all"
              />
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {filteredInsights.filter(i => !i.featured).map((item, index) => (
              ...
              >
                <div className="aspect-[16/10] rounded-[10px] overflow-hidden mb-6 shadow-sm border border-gray-100 dark:border-white/10 dark:border-white/10">
                ...
                <div className="flex items-center justify-between text-[10px] font-bold text-gray-400 uppercase tracking-widest border-t border-gray-100 dark:border-white/10 dark:border-white/10 pt-4">
                  <div className="flex items-center gap-2">
                    <Calendar size={12} />
                    {item.date}
                  </div>
                  <button className="flex items-center gap-1 text-[var(--c-fg)] group/link">
                    Read More
                  </button>
                </div>
              </motion.div>
        ...
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <form className="flex flex-col sm:flex-row gap-4 max-w-lg mx-auto">
            <input 
              ...
              className="flex-grow px-6 py-4 rounded-[10px] border border-gray-200 dark:border-white/5 dark:border-white/10 focus:outline-none focus:ring-2 focus:ring-[var(--c-lime)]/20 shadow-sm"
              required
            />
